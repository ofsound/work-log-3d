...
- Authenticated pages use Nuxt layouts built on `AppShell` (fixed header + `pt-8` clearance): **`main-simple`** (`px-6` padded scroll) for `/new`, `/projects`, `/tags`, `/reports`, `/settings`; **`main-workspace`** (full-height, no extra top padding) for `/sessions`, `/project/...`, `/tag/...`; **`main-bleed`** for `/` (hero only). **`default`** matches **`main-simple`** if a page omits `definePageMeta({ layout: ... })`.
